...
type RMetric struct {
	Host  string `json:"host"`
	Meta  string `json:"host_metadata"`
	//Value string `json:"value"`
	Clock int64  `json:"clock"`
}
...
func NewRegistryMetric(host, host_meta string) *RMetric {
	m := &RMetric{Host: host, Meta: host_meta, Clock:time.Now().Unix()}
	// use current time, if `clock` is not specified
...
func NewRPacket(data *RMetric, clock ...int64) *RPacket {
	p := &RPacket{Request: `active checks`, Host: data.Host,Meta:data.Meta}
	// use current time, if `clock` is not specified
	if p.Clock = time.Now().Unix(); len(clock) > 0 {
		p.Clock = int64(clock[0])
	}
...
func (s *Sender) Registy(packet *RPacket) (res []byte, err error) {
	conn, err := s.connect()
	if err != nil {
		return
	}
...
func (s *Sender) RegistryWithGivenSock(conn *net.TCPConn, packet *RPacket) (res []byte, err error) {
	if conn == nil{
...
func RegistryHosts(names []string,host string ,port int){
